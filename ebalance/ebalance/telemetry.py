...
"""
eBalance Telemetry - Anonymous Error Reporting to GitHub Issues

This module provides opt-in error reporting that creates GitHub issues
when critical errors occur. This helps the mn-frappe team improve eBalance.

Privacy:
- No personal data is collected
- No credentials, company data, or financial info is sent
...
GITHUB_REPO = "mn-frappe/ebalance"
GITHUB_API_URL = f"https://api.github.com/repos/{GITHUB_REPO}/issues"

# Rate limiting - max issues per day per site
MAX_ISSUES_PER_DAY = 5
CACHE_KEY = "ebalance_telemetry_issues_today"


def is_telemetry_enabled() -> bool:
    """Check if telemetry/error reporting is enabled in settings."""
    try:
        return bool(frappe.db.get_single_value("eBalance Settings", "enable_error_reporting"))
    except Exception:
        return False
...
def get_github_token() -> str | None:
    ...
    try:
        return frappe.db.get_single_value("eBalance Settings", "github_token")
    except Exception:
        return None
...
def get_environment_info() -> dict:
    ...
    info = {
        ...
        "ebalance_version": "unknown",
        "python_version": "unknown",
        "environment": "unknown",
    }

    try:
        from ebalance import __version__
        info["ebalance_version"] = __version__
    except Exception:
        try:
            # Fallback to pyproject.toml version
            info["ebalance_version"] = "1.3.0"
        except Exception:
            pass
    ...
    try:
        info["environment"] = frappe.db.get_single_value("eBalance Settings", "environment") or "unknown"
    except Exception:
        pass
...
def create_github_issue(
    ...
    if not token:
        frappe.log_error(
            "eBalance Telemetry: GitHub token not configured",
    ...
    try:
        ...
        frappe.log_error(f"Failed to create GitHub issue: {e}", "eBalance Telemetry")

    return None


def sanitize_data(data: dict) -> dict:
    """Remove sensitive data like credentials, company info, financial data."""
    sensitive_keys = (
        'password', 'token', 'secret', 'key', 'api_key', 'access_token',
        'org_regno', 'user_regno', 'company', 'amount', 'balance',
        'account', 'refresh_token', 'webhook_secret'
    )
...
| Property | Value |
|----------|-------|
| eBalance Version | `{env_info['ebalance_version']}` |
...
"""
    ...
    body += """---
*This issue was automatically created by eBalance Telemetry. The user has opted-in to anonymous error reporting.*
*No personal data, company information, or financial data is included in this report.*
"""
...
def report_error(
    ...
    if not check_rate_limit():
        frappe.log_error(
            "eBalance Telemetry: Rate limit exceeded for today",
    ...
    if token:
        ...
        if existing:
            frappe.log_error(
                f"eBalance Telemetry: Similar issue already exists: {existing}",
                "Telemetry Duplicate"
            )
    ...
    if result:
        ...
        frappe.log_error(
            f"eBalance Telemetry: Created issue {result.get('html_url')}",
            "Telemetry Success"
        )
...
def handle_exception(e: Exception, context: dict | None = None):
    ...
    # Always log locally
    frappe.log_error(stack_trace, f"eBalance Error: {error_message[:100]}")
...
def test_telemetry():
    ...
    if not is_telemetry_enabled():
        return {"status": "error", "message": _("Error reporting is not enabled in eBalance Settings")}
